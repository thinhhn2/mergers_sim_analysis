...
# -*- coding: utf-8 -*-
"""
Created on Mon May 20 11:48:13 2024
...
branch_idx = '0'
snapshot_idx = 100
halo = tree[branch_idx][str(snapshot_idx)]

if codetp == 'GADGET3' or codetp == 'AREPO':
    ds = yt.load(pfs[snapshot_idx], unit_base = {"length": (1.0, "Mpccm/h")})
else: 
...
for i in range(3):
    ...
    p = yt.ParticleProjectionPlot(ds, i, [("stars","particle_mass")], center = (com_kpc,'kpc'), width = (2*rvir_kpc, 'kpc'),density=True,fontsize=12)
    p.annotate_sphere(com.v.tolist(), radius=(radius500_kpc, "kpc"), circle_args={"color": "black"})
...
plt.savefig("surface_mass_density_%s_%s.png" % (branch_idx, snapshot_idx),dpi=600,bbox_inches='tight')
